...
Clip_Disp:		dw $0001,$0002
					...
					AND #$20 : BNE +
					;LDA !E4,x : CLC : ADC #$04 : STA !E4,x   	;\ tuna: this doesn't appear necessary, at least for the small fireball size
					;LDA !D8,x : CLC : ADC #$04 : STA !D8,x   	;/ it was causing problems with dynamically updating the firebar position so I've commented it out
					
+					LDA !extra_byte_2,x	 								;\
					STA !1534,x : BPL + 									;| process intial speed (Range: 00 -> 7F)
					AND #$7F : STA !1534,x 								;/ 8th bit is for some other property
					
+					LDA !extra_bits,x : AND #$04 						;\
					BEQ + 													;| process rotation direction
					LDA !1534,x : EOR #$FF : INC : STA !1534,x 	;/
					
+					LDA !extra_byte_3,x : TAY 							;\ process initial angle (Range: 00 -> FF)
					ASL #5 : ORA #$08 : STA !C2,x 					;| C2 holds the 3 LSBs? But shifted up, plus 8?
					TYA : LSR #3 : ORA !1504,x : STA !1504,x 		;/ 1504 holds the 5 MSBs...
					 															; probably the LSBs are fraction bits for fixed point
					 															; "ORA !1504,x" apparently pointless
...
+					LDA !extra_prop_1,x
					...
					CLC
					LDA !1528,x : AND #$20 : BEQ +
					SEC
+					LDA !1528,x : AND #$1F : BCC +
					ASL
...
START_SPRITE_CODE:	LDA !1528,x
					TAY : AND #$20
					LSR #3 : STA $D7 
					TYA : AND #$1F : STA $0C
					ASL #3
					LDY $D7 : BEQ + : ASL
+					;%SubOffScreen_New()    					; tuna: don't want these despawning offscreen in the boss fight
					
					LDA !1504,x : XBA : LDA !C2,x 			;\
					REP #$20 										;| put full angle in $04
					LSR #4 : STA $04 								;/
					DEC : LSR : SEP #$21 						;\
					SBC #$C0 : STA $0F 							;| not sure what 0F is
					LDA !extra_byte_1,x : AND #$C0 			;| 
					STA $0E 											;| pendulum or on/off state stuff
					SEC 												;|
					...
					JSL $018022 ; \ tuna: want the firebar to respond to X/Y speed changes (for the boss fight)
    				JSL $01801A ; /
...
Pendulum_Return:	LDY #$00
					AND #$04 : BNE +
					INY
+					LDA !1534,x : CLC : ADC !C2,x : STA !C2,x
...
ONOFF_Inversion:	LDA $14AF|!Base2 : BEQ ONOFF_Unrelated
					LDA !1534,x : EOR #$FF : INC : BRA +
					
ONOFF_Unrelated:	LDA !1534,x
+					BPL +
					INY
